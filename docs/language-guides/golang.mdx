...
<Tabs>
  <TabItem value="go" label="Go">
    ...
    type User struct {
        ...
        Email     string
        IsAdmin   bool
        createdAt time.Time  // lowercase = private (package-level)
    }
    ...
    // With positional arguments (discouraged)
    user2 := User{1, "Jane", "jane@example.com", true, time.Now()}
  ...
  <TabItem value="python" label="Python">
    ...
    # Usage
    user1 = User(1, "John", "john@example.com", False)
    user1.greet()     # "Hello, I'm John"
    user1.promote()   # Modifies user1
    ```
  </TabItem>
...
<Tabs>
  <TabItem value="go" label="Go">
    ...
    // Any type with a Greet() method implements Greeter
    type Person struct {
        Name string
    }
    ...
    // Both Person and Robot implement Greeter automatically
    ...
    // Empty interface (like Python's Any)
  ...
  <TabItem value="python" label="Python (ABC)">
    ...
    # Define an abstract class (like Go interface)
    class Greeter(ABC):
        @abstractmethod
        def greet(self) -> str:
            pass

    # Must explicitly inherit from Greeter
...
**Key Difference**: Go interfaces are satisfied implicitly. Python requires explicit inheritance from abstract classes.
