...
Two pointers algorithms are used to solve problems that involve sorted arrays, pairs, or sub-arrays/sub-strings efficiently to avoid the $$O(n^2)$$ complexity of nested loops.
...
The two pointers move independently. They can be from the same end or different ends of the array. [Sliding Window (Fixed)](/docs/algorithms/sliding-window) and [Sliding Window (Dynamic)](/docs/algorithms/sliding-window-dynamic) are variants of two pointers.
...
In summary, the two pointers technique is particularly useful for problems involving sorted arrays, pairs, and subarrays where you can maintain a linear traversal. If you find yourself needing to check all pairs of indices, consider whether the problem can be simplified by sorting the array or using a sliding window approach with two pointers.
