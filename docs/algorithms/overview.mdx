...
| [Sliding Window](./sliding-window) | Used for tracking a subset of elements in arrays/strings as a window that slides through the data. Great for finding subarrays or substrings meeting certain conditions. | $$O(n)$$ | $$O(1)$$ or $$O(k)$$ |
